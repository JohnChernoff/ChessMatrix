...
//TODO: clean up and move somewhere else (like main.js)?
function playMove(move,black_pov) {
  if (move !== null) {
    ...
    let chord = {
      bass: 24 + move.from.x,
      tenor: 36 + move.from.y,
      alto: 48 + move.to.x,
      soprano: 60 + move.to.y
    };
    let volume = .25;
    ...
    if (move.type === INST_CAPTURE || move.type === INST_CASTLING) {
      ...
      playNote(move.type,0,chord.bass,8,volume);
      playNote(move.type,0,chord.tenor,8,volume);
      playNote(move.type,0,chord.alto,8,volume);
      playNote(move.type,0,chord.soprano,8,volume);
      let extreme = 0;
      for (let p=0;p<Math.abs(move.piece);p++) {
        for(let i=0;i<dist;i++) {
          if (melody.patterns.length > i) {
            ...
            if (pitch <= 84 && pitch >= 24) melody.pitch = pitch; else extreme++;
      ...
      let dur = 1 + (Math.abs(move.piece)/2); //6 / (7 - Math.abs(move.piece)); //
      let dir = move.from.y - move.to.y;
      if (dir === 0) {
        //melody.patterns.push({ rhythm: dur, interval: 0}); if (melody.patterns.length > 12) melody.patterns.shift();
      }
      else {
        let last_interval = melody.patterns.length > 0 ? melody.patterns[melody.patterns.length-1].interval : 0;
        let interval = Math.round(((dist-1)/6) * 12) + (move.from.x - move.from.y);
        let up = move.piece < 0 ? dir < 0 :  dir > 0; if (black_pov) up = !up;
        let reversal = (Math.random() * last_interval) > 2;
        let undertow = (Math.random() * 144) < (melody.pitch - 24);
        if (!up || reversal || undertow) interval = -interval;
        melody.pitch += interval;
        if (melody.pitch > 84) melody.pitch = 84;
        else if (melody.pitch < 24) melody.pitch = 24;
        else {
          melody.patterns.push({ rhythm: dur, interval: interval});
          if (melody.patterns.length > 12) melody.patterns.shift();
        }
      }
      //console.log(melody);
      ...
      playNote(INST_HARMONY,0,toPitch,8,volume);
    }
  }
}
...
function octaveCheck(pitch,interval) {
  ...
  let new_octave = Math.floor((pitch + interval) / 12);
  let correction = ((octave-new_octave) * 12); //console.log(pitch + "," + interval + ", " + "Correction: " + correction);
  return correction + (pitch + interval);
}

function calcMoveDist(move) {
  return Math.sqrt(Math.pow(move.from.x - move.to.x,2) + Math.pow(move.from.y - move.to.y,2));
}
